...
# references "one_card"
# This fixture creates a card in board 1 and saves it in the database
@pytest.fixture
...
def three_cards(app):
    cards = [
        Card(message="Water the garden 🌷", likes_count=1),
        Card(message="Answer forgotten email 📧", likes_count=0),
        Card(message="Pay my outstanding tickets 😭", likes_count=2)
    ]
...
@pytest.fixture
def liked_card(app):
    card = Card(message="Explore Seattle 🏞", likes_count=0)
    db.session.add(card)
    db.session.commit()
    return card
